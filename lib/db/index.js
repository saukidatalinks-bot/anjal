...
export async function initDb() {
  ...
  // Client project portals (private project dashboards)
  await sql`
    CREATE TABLE IF NOT EXISTS clients (
      ...
      allow_final_payment BOOLEAN DEFAULT false,
      is_active BOOLEAN DEFAULT true,
      payment_bank VARCHAR(100) DEFAULT 'Opay',
      payment_account_number VARCHAR(50),
      payment_account_name VARCHAR(255),
      payment_note TEXT,
      portal_stage VARCHAR(30) DEFAULT 'review',
  ...
  const migrations = [
    `ALTER TABLE clients ADD COLUMN IF NOT EXISTS portal_stage VARCHAR(30) DEFAULT 'review'`,
    `ALTER TABLE clients ADD COLUMN IF NOT EXISTS quotation_content TEXT`,
    `ALTER TABLE clients ADD COLUMN IF NOT EXISTS contract_content TEXT`,
    `ALTER TABLE clients ADD COLUMN IF NOT EXISTS project_completed_at TIMESTAMP`,
  ]
  for (const m of migrations) {
    try { await sql.unsafe(m) } catch (_) { /* already exists */ }
  }

  // Client payment submissions + admin verification
  await sql`
  ...
  // Ensure is_approved column exists (migration for existing tables)
  try {
    await sql`ALTER TABLE testimonials ADD COLUMN is_approved BOOLEAN DEFAULT false`
  } catch (e) {
...
export async function seedDefaults() {
  ...
  // Seed a default private client portal for ADAM
  const adamMilestones = [
    {
      title: 'Discovery & Project Kickoff',
      description: 'Requirements confirmation, architecture, and implementation planning.',
      status: 'in_progress',
      due: 'Week 1',
    },
    {
      title: 'Web Platform Development',
      description: 'Responsive web app build, backend setup, and CMS/admin modules.',
      status: 'pending',
      due: 'Week 2',
    },
    {
      title: 'Android App + API Integration',
      description: 'Android build, API sync, push notifications, and release prep.',
      status: 'pending',
      due: 'Week 3',
    },
    {
      title: 'QA, Deployment & Handover',
      description: 'Testing, deployment, documentation, and final handover.',
      status: 'pending',
      due: 'Week 4',
    },
  ]
  ...
  await sql`
    INSERT INTO clients (
      ...
      'Valued Client',
      'adam@example.com',
      'Comprehensive Web & Mobile Application Development Package',
      'Website development, domain setup, Google Play account, Android app integration, and subsidized data vending API integration.',
      229500,
      'NGN',
      170,
      20,
      1,
      ${JSON.stringify(adamMilestones)}::jsonb,
      false,
      'Opay',
      '9024099561',
      'Ahmad Muhammad Jawa',
      'Project Lead account for milestone payments. Upload receipt after transfer for admin verification.'
    )
